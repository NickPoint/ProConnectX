          ...
          <StyledToolbar>
            ...
            {!user && (
              <Stack sx={{ cursor: "pointer" }} onClick={() => navigate("/")}>
                ...
                <Typography variant="body1" fontSize="0.75rem" component="span">
                  {t("platform.slogan")}
                </Typography>
              </Stack>
            )}
              ...
              {user && (
                <>
                  <Typography variant="h6">
                    {t(`header.user.greeting`, {
                      firstName: user?.activeProfile.displayName.split(" ")[0],
                    })}
                  </Typography>
                  {user.status && (
                    <Typography variant="body2">
                      {t(`header.user.status.${user.status}`)}
                    </Typography>
                  )}
                </>
              )}
